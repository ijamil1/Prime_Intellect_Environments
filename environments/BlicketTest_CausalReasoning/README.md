...
- **Rubric overview**: Reward combines blicket set Jaccard similarity (0.3), exploration efficiency (0.35), and hypotheses eliminated (0.35). Step budget utilization and format compliance are tracked but do not affect the reward.
...
| `exploration_efficiency` | 0.35 | `1 - (wasted / parseable)` — fraction of productive actions. Higher is better |
| `format_compliance` | 0.0 | tracked only |
| `hypotheses_eliminated` | 0.35 | Fraction of hypotheses eliminated vs. the optimal greedy info-gain agent. Higher is better |
